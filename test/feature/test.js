...
module.exports.test = {
	...
	'get none': function(test) {
		client.get('/test/none', function(err, req, res, obj) {
			//test.expect(1);
			//test.equal('my_key', obj.key);
			test.done();
			console.log(obj);
	...
	'put none': function(test) {
		var p = {
			value: 'my_value_modify'
		};
		client.put('/test/none', p, function(err, req, res, obj) {
			test.done();
			console.log(obj);
		});
	},
	...
	'del none': function(test) {
		client.del('/test/none', function(err, req, res, obj) {
			test.done();
			console.log(obj);
		});
	}
