...
Res.prototype.find = function(arg) {
	...
	var fn = ('function' == typeof arg) ? arg : function(req, res, next) {
			...
			da.exec(function(err, doc) {
				if (err) {
					next(req, res, error(500, err.message));
				} else if (opt.page) {
 ...
 * @param {Function, Object} `arg`
 *	options:
 *		- {Object, Boolean} `query` query condition
 *		- {Object} `setter` enable selection on fields
 *		- {Boolean} `[multi]` default to true
 *		- {Boolean} `[upsert]` default to false
 * @public api
 */
Res.prototype.update = function(arg) {
	...
	var fn = ('function' == typeof arg) ? arg : function(req, res, next) {
			var cond, set, ext;
			cond = gears.conditions(req, opt.query);
			// bad conditions
			if (cond === null) {
				next(req, res, error(400, 'Query Condition Required'));
				return;
			};
			// error in query
			if (E(cond)) {
			...
			set = gears.setter(req, opt.setter);
			ext = {
				multi: opt.multi || true,
				upsert: opt.upsert || false
			};
			this.model.update(cond, set, ext, function(err) {
				err = err ? error(500) : null;
				next(req, res, err);
			});
		};
...
Res.prototype.remove = function(arg) {
	var opt = ('object' == typeof arg) ? arg : {};
	var fn = ('function' == typeof arg) ? arg : function(req, res, next) {
			var cond = gears.conditions(req, opt.query);
			// bad conditions
			if (cond === null) {
				next(req, res, error(400, 'Query Condition Required'));
				return;
			...
			this.model.remove(cond, function(err) {
				err = err ? error(500) : null;
				next(req, res, err);
			});
		};
