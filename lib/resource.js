	...
	if (action) {
		...
		// or default handler
			...
			index.handler = function(req, res, next) {
				...
				if (action.filter) {
					var select = null;
					// get customized filter from request params
					if (action.filter.customizable) {
						select = util.param(req, '$filter', function(value) {
							return value ? value.replace(/\,/g, ' ') : null;
						});
					}
					// or set to default filter
					if (!select && action.filter['default']) {
						select = action.filter['default'];
					}
					// default filter
					else {
						select = {
				...
				if (action.sort) {
					...
					if (action.sort.customizable) {
						by = util.param(req, '$sort', function(value) {
							return value ? value.replace(/\,/g, ' ') : null;
						});
					}
					// or set to default sorting
					if (!by && action.sort['default']) {
						by = action.sort['default'];
					}
					// sort
					if (by) {
						query.sort(by);
