...
RestCook.prototype.resource = function(key, config) {
	...
	function _setup(from, to, field, type, desc, fallback) {
	...
	_setup(config, _res, 'collection', 'string', 'config.collection', function() {
	...
	_setup(config, _res, 'pk', 'string', 'config.pk', function() {
		return '_id';
	});
...
/**
 * Build actions for resource.
 *
 * @param resource {Object}
 * @private
 */
RestCook.prototype.__buildAction = function(resource) {

	assert.object(resource, 'resource');
	assert.object(resource.action, 'resource.action');

	var action = resource.action;
	if (action.index) {
		this.__buildIndex(resource);
	}
	if (action.post) {
		this.__buildPost(resource);
	}
	if (action.get) {
		this.__buildGet(resource);
	}
	if (action.put) {
		this.__buildPut(resource);
	}
	if (action.del) {
		this.__buildDel(resource);
	}
};
 ...
 * @private
 */
RestCook.prototype.__buildIndex = function(resource) {
	...
	if (action.handler) {
		this._server.get(action.route, function(req, res) {
			action.handler(req, res, function(err, result, responded) {
				if (!responded) {
					if (err) {
						res.json(err.code ? err.code : 500, err);
					} else {
						res.json(200, result);
					}
				}
 ...
 * @private
 */
RestCook.prototype.__buildPost = function(action) {
	// body...
};
 ...
 * @private
 */
RestCook.prototype.__buildGet = function(action) {
	// body...
};
 ...
 * @private
 */
RestCook.prototype.__buildPut = function(action) {
	// body...
}
 ...
 * @private
 */
RestCook.prototype.__buildDel = function(action) {
	// body...
}
